...
class Vertex {
   ...
   */
  toCanvasCords(pixelSize: number): Vertex {
    return new Vertex(
...
class Polygon {
   ...
   */
  isPointInPolygon(point: Vertex): boolean {
...
function main() {
  ...
  const poligonos = Array<Polygon>();
  const canvas: any = document.querySelector("canvas");
  const ctx: any = canvas.getContext("2d");
  const buttons = document.querySelectorAll("button");
  const inputs = document.querySelectorAll("input");

  let previousPoint: null | Vertex = null;
  let isDrawing = false;
   ...
   */
  function blockOthers(except: string) {
   ...
   */
  function fillpolly(polygon: Polygon) {
    ...
    const scanlines: number[][] = Array.from({ length: Ns }, () => []);
    ...
    for (let i = 0; i < Ns; i++) scanlines[i].sort((a, b) => a - b);
    ...
    for (let i = 0; i < Ns; i++) {
      for (let ii = 0; ii < scanlines[i].length; ii += 2) {
        ...
        const p1 = new Vertex(...min),
          p2 = new Vertex(...max);

        drawLine(
          p1.toCanvasCords(pixelSize),
          p2.toCanvasCords(pixelSize),
          polygon.getColor(),
          pixelSize
        );
      }
   ...
   */
  function pointToEdgeDistance(p: Vertex, v1: Vertex, v2: Vertex): number {
    const x1 = v1.x,
      y1 = v1.y,
      x2 = v2.x,
      y2 = v2.y;
    ...
    if (dotProduct1 < 0) return Math.sqrt((px - x1) ** 2 + (py - y1) ** 2);
    else if (dotProduct2 < 0) return Math.sqrt((px - x2) ** 2 + (py - y2) ** 2);
   ...
   */
  function findClosestEdge(polygon: Polygon[], pointX: number, pointY: number): Polygon | null {
    let closestPolygon: Polygon | null = null;
   ...
   */
  function redraw(withEdges: boolean = false): void {
    initializeCanvas();
    ...
    for (let i = 0; i < poligonos.length; i++) {
      ...
      for (let ii = 0; ii < cords.length; ii++) {
        ...
        const v2 = new Vertex(
          cords[(ii + 1) % cords.length].x,
          cords[(ii + 1) % cords.length].y
        );

        drawLine(
          v1.toCanvasCords(pixelSize),
          v2.toCanvasCords(pixelSize),
          "yellow",
          pixelSize
        );
      }
  ...
  canvas.addEventListener("click", (event: any) => {
    ...
    if (isInspecting) {
      ...
      const selectedPolygons = poligonos.filter(
        (value, index) => inside[index]
      );

      const closestPolygon: null | Polygon = findClosestEdge(selectedPolygons, x, y);

      if (closestPolygon) {
        if (isExcluding) {
          const idx = poligonos.indexOf(closestPolygon);
          poligonos.splice(idx, 1);
        } else closestPolygon.setColor(selectedColor);
      }
    ...
    if (previousPoint)
      drawLine(
        previousPoint,
        currentPoint.toCanvasCords(pixelSize),
        selectedColor,
        pixelSize
      );
    else poligonos.push(new Polygon());

    const lastPol = poligonos.at(-1) as Polygon;
  ...
  $("#btnAdicionarPoligono").on("click", () => {
    ...
    const lastPolDrawed: any = poligonos.at(-1);
  ...
  $("#colorPicker").on("change", function () {
    selectedColor = $(this).val() as string;
  });
